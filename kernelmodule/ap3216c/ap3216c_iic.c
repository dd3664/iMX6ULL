 ...
 *@retrun: 操作结果
*/
static int ap3216c_read_regs(AP3216cDev *dev, u8 reg, void *val, int len)
{
	int ret;
 ...
 *@return: 操作结果
*/
static int ap3216c_write_regs(AP3216cDev *dev, u8 reg, u8 *val, u8 len)
{
	u8 b[256] = {0};
	struct i2c_msg msg;
	struct i2c_client *client = (struct i2c_client*)dev->private_data;
 ...
 *retrun: 读取到的寄存器值
*/
static unsigned char ap3216c_read_reg(AP3216cDev *dev, u8 reg)
{
	u8 data = 0;
 ...
 *@return: 无
*/
static void ap3216c_write_reg(AP3216cDev *dev, u8 reg, u8 data)
{
	u8 buf = 0;
	buf = data;
...
/*
 *@description: 读取AP3216C的数据，读取原始数据
 *@param-dev: ap3216c设备
 *@return: 无
*/
static void ap3216c_read_data(AP3216cDev *dev)
{
	unsigned char i = 0;
...
static ssize_t ap3216c_read(struct file *filp, char __user *buf, size_t cnt, loff_t *offt)
{
	short data[3];
	long err = 0;
	AP3216cDev *dev = (AP3216cDev *)filp->private_data;
